...
let allTabs = "";
const facetTabName = "content_type_ss";
const allTabsLabel = "All";
  ...
  hooks: {
    beforeSearch: (props) => {
      ...
      if (!firstSearch) {
        ...
        if (
          propsCopy.facets.length > 0 &&
          propsCopy.facets[0].value == allTabsLabel
        ) {
          propsCopy.facets = [];
        }
      }
...
searchstax.addFacetsWidget("searchstax-facet-tabs-container", {
  facetingType: "tabs",
  specificFacets: [facetTabName],
  itemsPerPageDesktop: 99,
  ...
  templates: {
    ...
    // Make mobile use the same layout as desktop
    mainTemplateMobile: {
      template: `
          {{#hasResultsOrExternalPromotions}}
    ...
    facetItemContainerTemplate: {
      template: `
              <div>
                  ...
                  <div class="searchstax-facet-values-container searchstax-facet-tabs-values-container" aria-live="polite">

                  </div>
              </div>
          `,
      facetListTitleContainerClass: `searchstax-facet-title-container`,
      facetListContainerClass: `searchstax-facet-tabs-values-container`,
    },
  },
});
